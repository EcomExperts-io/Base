...
if (!customElements.get('product-info')) {
  class ProductInfo extends HTMLElement {
    ...
    setupEventListeners() {
      ...
      document.addEventListener('cart:change', this.onCartUpdate.bind(this));
    }
...
class ProductInfo extends HTMLElement {
  onCartUpdate(e) {
    ...
    // cart:change only fires on successful requests
    // detail: { action, payload, response, cart, previousCart, sections, source }
    if (e.detail.action === 'add') {
      // Add the CSS class to the "body" tag
      document.body.classList.add('js-item-added');

      // Dispatch a custom event for other components
      // (for adds, detail.response is the added line item from /cart/add.js)
  ...
  setupEventListeners() {
    // Listen to the cart engine's events (assets/cart.js)
    document.addEventListener('cart:change', this.onCartUpdate.bind(this));
...
**Cart mutations go through the `window.Cart` API (assets/cart.js):**

```javascript
// Add to cart — pass the product form's FormData
Cart.add(new FormData(form), form);

// Change a line quantity
Cart.change({ line: 1, quantity: 2 }, sourceElement);

// Update cart note or attributes
Cart.update({ note: 'Gift wrap please' }, sourceElement);

// Re-fetch state + section HTML after external cart changes
Cart.refresh();
```

- All mutations are serialized through an internal queue, so rapid calls can't race.
- Success dispatches `cart:change` on `document`; failures dispatch `cart:error` with `detail.error: { status, message, description }`.
- Current cart state is always available at `window.Cart.state`.
