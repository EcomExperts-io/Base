...
`padding_bottom_mobile`, `color_scheme`, a `presets` entry, `t:` keys
...
2. **The settings contract is already satisfied.** `padding_top`, `padding_bottom`, `padding_top_mobile`, `padding_bottom_mobile`, `color_scheme` and a `presets` entry are present. Keep them — the pre-commit gate blocks a new section without them, and the merchant-facing contract is judged separately from visual fidelity.
${notes_tail}
"""
)
